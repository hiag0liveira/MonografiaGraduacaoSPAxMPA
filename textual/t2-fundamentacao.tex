...
\subsection{Node.js}
\label{subsec:nodejs}
O \english{Node.js} é um ambiente de execução JavaScript baseado no motor V8 do Google Chrome, projetado para executar código no lado do servidor. Sua arquitetura orientada a eventos e modelo de entrada/saída não bloqueante possibilitam a construção de aplicações escaláveis e de alto desempenho \cite{nodejs_docs}. Com um vasto ecossistema de pacotes disponível por meio do \english{Node Package Manager (npm)}, o Node.js é amplamente utilizado no desenvolvimento de \english{back-ends}, APIs e na integração com frameworks modernos de \english{front-end}.

\subsection{Next.js}
\label{subsec:nextjs}
O \english{Next.js} é um \english{meta-framework} para React, desenvolvido pela empresa Vercel, que oferece suporte nativo a múltiplas estratégias de renderização, incluindo \acrshort{ssr}, \acrshort{ssg} e \acrshort{isr}. Ele abstrai a complexidade da configuração de um projeto React com renderização no servidor, permitindo que desenvolvedores definam o comportamento de cada página de acordo com necessidades específicas de desempenho e \acrshort{seo} \cite{vercel_next_docs}. Entre suas funcionalidades destacam-se o \english{routing} baseado em arquivos, a otimização automática de imagens e a possibilidade de criação de APIs internas, o que o torna uma solução completa para o desenvolvimento de aplicações web modernas.

\subsection{React}
\label{subsec:react}
O \english{React} é uma biblioteca JavaScript de código aberto, desenvolvida pelo Facebook (atualmente Meta), voltada para a criação de interfaces de usuário baseadas em componentes reutilizáveis. Sua abordagem declarativa e o uso do \english{Virtual DOM} permitem atualizações eficientes na interface, reduzindo o custo de renderização e melhorando a experiência do usuário \cite{react_docs}. Além disso, o React serve como base para diversos frameworks e meta-frameworks, como o \english{Next.js}, possibilitando tanto a renderização no lado do cliente (\acrshort{csr}) quanto no lado do servidor (\acrshort{ssr}), de acordo com as necessidades do projeto.
